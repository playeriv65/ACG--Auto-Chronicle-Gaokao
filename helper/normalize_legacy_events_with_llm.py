...
def _load_events_from_db(db_path: Path) -> list[dict[str, Any]]:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        rows = cur.fetchall()
    return [{"id": int(r[0]), "season": str(r[1]), "description": str(r[2]), "effect": str(r[3] or "")} for r in rows]
...
def _build_prompt(batch: list[dict[str, Any]]) -> str:
    payload = {"items": [{"id": item["id"], "description": item["description"]} for item in batch]}
    return (
        ...
        for i in range(start, len(events), batch_size):
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    response_text = msg.content or getattr(msg, "reasoning_content", None)
                    ...
                    if attempt == MAX_RETRIES - 1:
                        raise RuntimeError(f"Batch {i // batch_size + 1} failed: {exc}") from exc
                    prompt += (
                        "\n\n上轮输出不合规，请严格修正："
            ...
            if cleaned_items is None:
                raise RuntimeError(f"No cleaned items for batch at index {i}: {last_error}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Normalize legacy DB events to {p1..} templates via LLM.")
    parser.add_argument("--batch-size", type=int, default=20)
    parser.add_argument("--sleep", type=float, default=0.2)
    parser.add_argument("--model", type=str, default=None)
    parser.add_argument("--reset", action="store_true")
    args = parser.parse_args()
    run(batch_size=args.batch_size, sleep_sec=args.sleep, model=args.model, reset=args.reset)
