...
def test_generate_plan_in_isolated_workspace(
    ...
    world_settings = assert_json_shape(
        isolated_workspace / "world_settings.json", ["meta", "characters"]
    )
    assert (
        isinstance(world_settings["characters"], list) and world_settings["characters"]
    ), "PLAN_GENERATION_FAILED: empty characters"
    ...
    assert "background" not in world_settings["characters"][0], (
        "PLAN_GENERATION_FAILED: background must be removed"
    )

    weekly_script = assert_json_shape(
        isolated_workspace / "weekly_script.json", ["meta", "weeks"]
    )
    assert isinstance(weekly_script["weeks"], dict) and weekly_script["weeks"], (
        "PLAN_GENERATION_FAILED: empty weeks"
    )
    validated_weekly = WeeklyScript.model_validate(weekly_script)
    first_week = next(iter(validated_weekly.weeks.values()))
    assert "排名:" in first_week.rankings.mc_report, (
        "PLAN_GENERATION_FAILED: mc_report should be rendered text"
    )
